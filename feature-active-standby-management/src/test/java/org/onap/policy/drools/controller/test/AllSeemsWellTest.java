...
public class AllSeemsWellTest {
    ...
    public void testAllSeemsWell() throws Exception {
        ...
        for (StateManagementFeatureAPI feature : StateManagementFeatureAPI.impl.getList()) {
            ...
            logger.debug("testAllSeemsWell stateManagementFeature.getResourceName(): {}", stateManagementFeatureAPI.getResourceName());
            break;
        }
        ...
        waitForCondition(()-> smf.getStandbyStatus().equals(StateManagement.PROVIDING_SERVICE),  RESUMED_ELECTION_HANDLER_SLEEP_TIME_SEC);
    ...
    private void waitForCondition(Supplier<Boolean> testCondition, int secTimeout) throws InterruptedException {
        int iterations = 0;
        while (!testCondition.get() && iterations<secTimeout)
        {
            iterations++;
            Thread.sleep(1000);
        }
