...
public class AllSeemsWellTest {
    ...
    private void waitForCondition(BooleanSupplier testCondition, int timeoutInSeconds) throws InterruptedException {
        int maxIterations = timeoutInSeconds * 10;
        int iterations = 0;
        while (!testCondition.getAsBoolean() && iterations < maxIterations) {
            iterations++;
            testTime.waitFor(100);
        }
    }
}
