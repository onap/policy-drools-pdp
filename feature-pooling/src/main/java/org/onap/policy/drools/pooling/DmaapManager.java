...
public class DmaapManager {
     ...
     */
    public DmaapManager(String topic, TypedProperties props) throws PoolingFeatureException {

        logger.info("initializing bus for topic {}", topic);

        try {
            this.topic = topic;
            this.sources = TopicEndpoint.manager.addTopicSources(props.getProperties());
            this.sinks = TopicEndpoint.manager.addTopicSinks(props.getProperties());
