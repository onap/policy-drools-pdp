...
public class QueryState extends ProcessingState {
	...
	/**
	 * UUID of hosts that have sent an "Identification" message. Always includes
	 * this host.
	 */
	...
	public QueryState(State oldState, String leader, BucketAssignments assignments) {
		 ...
		 */
		addTimer(new Timer(IDENTIFICATION_MS) {
			...
			public State fire() {
				...
				/*
				 * We aren't the leader, thus we'll now wait for the leader to
				 * take charge.
				 */
				...
				// create a new timer, in case leader never takes charge
				awaitLeader();
