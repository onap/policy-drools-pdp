...
public class PoolingFeature implements PolicyControllerFeatureAPI, PolicySessionFeatureAPI {
    ...
    public PolicyController beforeCreate(String name, Properties properties) {
        ...
        Properties props = getControllerPoolingProperties(name);
        
        if (isFeatureEnabled(props)) {

            // validate the properties
            try {
                new PoolingProperties(props);
    ...
    public boolean afterCreate(PolicyController controller) {
        logger.info("pooling enabled for {}", controller.getName());

        Properties props = getControllerPoolingProperties(controller.getName());
    ...
    /**
     * Determines if the pooling feature is enabled by looking for the feature-enabled
     * property.
     * @param props set of properties to examine, or {@code null} if there are no properties
     * @return {@code true} if the feature is enabled, {@code false} otherwise
     */
    public boolean isFeatureEnabled(Properties props) {
        if(props == null) {
            return false;
        }
        
        String enabled = props.getProperty(PoolingProperties.FEATURE_ENABLED);
     ...
     */
    private Properties getControllerPoolingProperties(String name) {
        if(featProps == null) {
            throw new IllegalStateException("feature properties have not been loaded");
        }
