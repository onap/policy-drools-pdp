...
public class PoolingManagerImpl implements PoolingManager, TopicListener {
     ...
     * the same.
     */
    private State current = new IdleState(this);
    ...
    public void afterUnlock() {
        synchronized (curLocker) {
            if (controller.isAlive() && current instanceof IdleState) {
                changeState(new StartState(this));
            }
        }
    }
     ...
     * events, thus we need to forward them. Otherwise, we immediately return
     * and let {@link #beforeInsert(Object, String, String, Object)
     * beforeInsert()} handle it instead.
     * 
     * @param protocol
     * @param topic2
     * @param event
