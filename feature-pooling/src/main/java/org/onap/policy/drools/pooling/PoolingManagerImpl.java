...
public class PoolingManagerImpl implements PoolingManager {
    ...
    /**
     * Topic source whose filter is to be manipulated. Initialized by {@link #afterStart()}.
     */
    private FilteredTopicSource topicSource;

    /**
     * Type used when delivering to the internal topic. Initialized by {@link #afterStart()}.
     */
    private CommInfrastructure busType;

    /**
     * Lock used while updating {@link #current}. In general, public methods must use this, while private methods assume
     ...
     * <p/>
     * This uses a finite state machine, wherein the state object contains all of the data relevant to that state. Each
     * state object has a process() method, specific to each type of {@link Message} subclass. The method returns the
     * next state object, or {@code null} if the state is to remain the same.
     */
    ...
    /**
     * {@code True} if events offered by the controller should be intercepted, {@code false} otherwise.
     */
    ...
    public void internalTopicFailed() {
        ...
        /*
         * We don't want to build up items in our queue if we can't forward them to other hosts, so we just stop the
         * controller.
         * 
    ...
    private void changeState(State newState) {
        /*
         * Note: have to check "current != null", too, because we may be using the defaultHandler, in which case, we
         * really don't want to change states.
         */
     ...
     * @param topic2
     * @param event
     * @return {@code true} if the event was handled, {@code false} if the controller should handle it
     */
    ...
    public void beforeStart() {
        synchronized (curLocker) {
            if (scheduler == null) {
                ...
                /*
                 * Only a handful of timers at any moment, thus we can afford to take the time to remove them when
                 * they're cancelled.
                 */
        ...
        for (TopicSource src : controller.getTopicSources()) {
            if (topic.equals(src.getTopic())) {
                if (src instanceof FilteredTopicSource) {
                    ...
                    throw new PoolingFeatureException("controller " + controller.getName() + " topic source " + topic
                                    + " is not filtered");
                }
            }
        }
    ...
    public void beforeStop() {
        ...
        synchronized (curLocker) {
            ...
            if (current != null) {
                ...
                /*
                 * Need a brief delay here to allow "offline" message to be transmitted?
                 */
            }
        }
