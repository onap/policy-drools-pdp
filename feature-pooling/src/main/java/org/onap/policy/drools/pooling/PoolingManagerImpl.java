...
public class PoolingManagerImpl implements PoolingManager, TopicListener {
    ...
    /**
     * Default state used when {@link #current} is {@code null}.
     */
    private final State defaultState = new IdleState(this);
    ...
    public PoolingManagerImpl(PolicyController controller, PoolingProperties props) {
        ...
        try {
            ...
            this.dmaapMgr = new DmaapManager(topic, props.getSource());
    ...
    private void changeState(State newState) {
         ...
         * the defaultState, in which case, we really don't want to change
         * states.
         */
     ...
     * engine. If the controller is locked, then it isn't processing events,
     * thus we need to forward them. Otherwise, we just return immediately and
     * let {@link #beforeInsert(Object, String, String, Object) beforeInsert()}
     * handle it instead.
     * 
     * @param protocol
     * @param topic2
    ...
    private Object decodeEvent(String topic2, String event) {
        ...
        if (!EventProtocolCoder.manager.isDecodingSupported(drools.getGroupId(), drools.getArtifactId(), topic2)) {
        ...
        try {
            return EventProtocolCoder.manager.decode(drools.getGroupId(), drools.getArtifactId(), topic2, event);

        } catch (UnsupportedOperationException uoe) {
            logger.debug("{}: DECODE FAILED: {} <- {} because of {}", drools, topic2, event, uoe.getMessage(), uoe);
            return null;

        } catch (Exception e) {
            logger.warn("{}: DECODE FAILED: {} <- {} because of {}", drools, topic2, event, e.getMessage(), e);
            return null;
        }
    }
    ...
    private void handleInternal(String event) {
        ...
        try {
            ...
            State hdlr = (current != null ? current : defaultState);
    ...
    /**
     * Indicates that the controller is about to be locked. Enters the stopped
     * state.
     */
    ...
    /**
     * Enters the start state, if not already running.
     */
    ...
    /**
     * Enters the stopped state, if currently running.
     */
    private void enterStoppedState() {
        if (current != null) {
            ...
            setFilter(defaultState.getFilter());
        }
    }
