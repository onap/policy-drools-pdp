...
public class ProcessingState extends State {
    ...
    /**
     * If the message source is an acceptable leader (i.e., <= {@link #leader}), then it transitions to the
     * {@link ActiveState} (or {@link InactiveState}, if this host has no bucket assignments).
     */
    @Override
    ...
    /**
     * Removes excess hosts from the set of available hosts. Assumes "this" host is the leader, and thus appears as the
     * first host in the set.
     * 
    ...
    private void removeExcessHosts(int maxHosts, TreeSet<String> avail) {
        while (avail.size() > maxHosts) {
            /*
             * Don't remove this host, as it's the leader. Since the leader is always at the front of the sorted set,
             * we'll just pick off hosts from the back of the set.
             */
    ...
    /**
     * Converts an array of bucket assignments to a collection of {@link HostBucket} objects.
     * 
    ...
    /**
     * Assigns dead buckets to available hosts. Removes dead hosts from the collection.
     * 
     * @param avail available hosts
    ...
    /**
     * Extracts dead hosts from a collection. Removes dead hosts from the collection. The collection is permitted to
     * contain a {@link HostBucket} with a {@code null} host, which identifies unassigned buckets.
     * 
     * @param avail hosts that are still available
     * @param coll collection from which to extract the hosts
    ...
    /**
     * Re-balances the buckets, taking from those that have a larger count and giving to those that have a smaller
     * count. Populates an output array with the new assignments.
     * 
    ...
    private void rebalanceBuckets(Collection<HostBucket> coll, String[] bucket2host) {
        ...
        /*
         * Note: as we'll be updating the counts of the HostBucket objects in each of these queues, we need to create a
         * clone of each of the objects for one of the queues.
         * 
         * In addition, we don't bother updating a given host's data in both queues. As we stop as soon as the counts
         * are within 1 of each other, we know that we'll never visit the items at the other end of the queue.
         * Consequently, it's ok that we don't update their counts. This allows us to simply use the remove() method,
         * which is O(log(N)), instead of the remove(Object) method, which is O(N).
         */
     ...
     * 
     * @param queue queue to which to add the items
     * @param coll collection of items to be cloned
     */
