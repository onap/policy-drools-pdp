...
public class ProcessingState extends State {
     ...
     * @param assignments current known, valid assignments, or {@code null} if
     *        no assignments are known yet
    ...
    public State process(Leader msg) {
        ...
        if (newLeader != null && newLeader.compareTo(leader) <= 0) {
            ...
            if (asgn.hasAssignment(getHost())) {
                return goActive(newLeader, asgn);
    ...
    protected State becomeLeader(SortedSet<String> alive) {
        ...
        return goActive(leader, msg.getAssignments());
    }
