...
public class BasicStateTester {
     ...
     * @return the (delay,task) pair that was scheduled
     */
    protected Pair<Long,StateTimerTask> captureSchedule(int index1) {
        int index = index1 - 1;
        ...
        verify(mgr, times(index1)).schedule(delayCap.capture(), taskCap.capture());
        
        assertTrue(taskCap.getAllValues().size() >= index1);
