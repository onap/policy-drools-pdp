...
public class BasicStateTester {
    ...
    protected static final int STD_LEADER_MS = STD_IDENTIFICATION_MS + 1;
    ...
    public void setUp() throws Exception {
        ...
        when(props.getLeaderMs()).thenReturn(STD_LEADER_MS);
    ...
    protected <T extends Message> T captureAdminMessage(Class<T> clazz, int index) {
        ...
        verify(mgr, atLeast(index + 1)).publishAdmin(msgCap.capture());

        return msgCap.getAllValues().get(index);
    }
    ...
    protected <T extends Message> Pair<String, T> capturePublishedMessage(Class<T> clazz, int index) {
        ...
        verify(mgr, atLeast(index + 1)).publish(channelCap.capture(), msgCap.capture());

        return new Pair<>(channelCap.getAllValues().get(index), msgCap.getAllValues().get(index));
    }
    ...
    protected Pair<Long, StateTimerTask> captureSchedule() {
        return captureSchedule(0);
    }

    /**
     * Captures the arguments to a call to schedule().
     * 
     * @param index1 index of the item to be captured
     * @return the (delay,task) pair that was scheduled
     */
    protected Pair<Long, StateTimerTask> captureSchedule(int index) {
        ...
        verify(mgr, atLeast(index + 1)).schedule(delayCap.capture(), taskCap.capture());

        return new Pair<>(delayCap.getAllValues().get(index), taskCap.getAllValues().get(index));
    }

    /**
     * Captures the arguments to a call to scheduleWithFixedDelay().
     * 
     * @param index1 index of the item to be captured
     * @return the (delay,task) pair that was scheduled
     */
    protected Triple<Long, Long, StateTimerTask> captureScheduleWithFixedDelay(int index) {

        ArgumentCaptor<Long> initDelayCap = ArgumentCaptor.forClass(Long.class);
        ArgumentCaptor<Long> delayCap = ArgumentCaptor.forClass(Long.class);
        ArgumentCaptor<StateTimerTask> taskCap = ArgumentCaptor.forClass(StateTimerTask.class);

        verify(mgr, atLeast(index + 1)).scheduleWithFixedDelay(initDelayCap.capture(), delayCap.capture(), taskCap.capture());

        return new Triple<>(initDelayCap.getAllValues().get(index), delayCap.getAllValues().get(index),
                        taskCap.getAllValues().get(index));
    }
