...
public class ControllerLoggingTest {
    ...
    /**
     * These properties are for installing a test artifact that the drools
     * controller can fetch while testing.
     */
    ...
    /**
     * These properties are used for the Policy Controller to point to the test
     * artifact.
     */
    ...
    /**
     * A custom appender used to intercept events and add them to a list of
     * events that the junits can use to determine logging was successful.
     */
    ...
    public static void setUp() throws IOException {
        KieUtils.installArtifact(Paths.get(JUNIT_KMODULE_PATH).toFile(), Paths
                .get(JUNIT_KMODULE_POM_PATH).toFile(), JUNIT_KJAR_DRL_PATH,
                Paths.get(JUNIT_KMODULE_DRL_PATH).toFile());

        controllerProps = new Properties();
        controllerProps.put(DroolsProperties.PROPERTY_CONTROLLER_NAME,
                TEST_CONTROLLER_NAME);
        controllerProps.put(DroolsProperties.RULES_GROUPID, TEST_GROUP_ID);
        controllerProps.put(DroolsProperties.RULES_ARTIFACTID,
                TEST_ARTIFACT_ID);
        ...
        message = "{\"requestID\":\"38adde30-cc22-11e8-a8d5-f2801f1b9fd1\",\"entity\":\"controller\","
            + "\"controllers\":[{\"name\":\"test-controller\",\"drools\":{\"groupId\":\"org.onap.policy.drools.test\","
    ...
    /**
     * Runs after every test case to clean up the events added to the event list
     * during unit test.
     */
    @After
    ...
    /**
     * Obtains the sequence number of the controller logging feature. This
     * should return 1000.
     */
    @Test
    ...
    /**
     * Asserts that the controller-test logger appends the incoming message to
     * the event list.
     */
    @Test
    ...
    /**
     * Asserts that the controller-test logger appends the outgoing message to
     * the event list.
     */
    @Test
    public void afterDeliverSuccess() {
        ...
        DroolsController droolsController = DroolsController.factory.get(
                TEST_GROUP_ID, TEST_ARTIFACT_ID, TEST_VERSION);

        TopicSink sinkTopic = new InlineUebTopicSink(BusTopicParams.builder()
                .topic(TEST_TOPIC).servers(Arrays.asList(TEST_SERVER)).build());

        nlf.afterDeliver(droolsController, sinkTopic, null,
                "{\"test\":\"test\"}", true);
    ...
    /**
     * Asserts that the controller-test logger does not append the outgoing
     * message to the event list if there was a failure.
     */
    @Test
    ...
    /**
     * Asserts that the controller logging feature can log the messages to the
     * proper controller based on the message containing the controller name.
     */
    @Test
    ...
    /**
     * Asserts that the controller logging feature can skip logging messages
     * that don't contain the controller names in it.
     */
    @Test
