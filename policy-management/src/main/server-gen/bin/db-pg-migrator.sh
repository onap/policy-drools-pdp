...
function ensure_metadata
{
    ...
    sql="SELECT 'CREATE DATABASE ${METADATA_DB}' WHERE NOT EXISTS (SELECT FROM pg_database WHERE datname = '${METADATA_DB}');"
    ${PGSQL} --command "${sql}"
    rc=$?
    if [ ${rc} -ne 0 ]; then
        return ${rc}
...
function ensure_metadata_schema
{
    ...
    sql="SELECT 'CREATE DATABASE ${SCHEMA_DB}' WHERE NOT EXISTS (SELECT FROM pg_database WHERE datname = '${SCHEMA_DB}');"
    ${PGSQL} --command "${sql}"
    return $?
}
...
function current_release
{
    ...
    CURRENT_RELEASE=$(${PGSQL} -tAc "${query}")
...
function track_script
{
    ...
    sql=${sql}"ON CONFLICT (script) DO UPDATE SET operation = EXCLUDED.operation,success = EXCLUDED.success,atTime = EXCLUDED.atTime;"
...
PGSQL="${PGSQL_CMD} -U ${SQL_USER} -W ${SQL_PASSWORD} -h ${SQL_HOST} -p ${SQL_PORT}"

if ! ${PGSQL} --execute "SELECT datname FROM pg_database;" > /dev/null 2>&1; then
...
for dbPath in ${SCHEMA_S}; do
    ...
    SCHEMA_DB="\`${SCHEMA}\`"
    UPGRADE_DIR="${MIGRATION_DIR}"/"${SCHEMA}"/psql
