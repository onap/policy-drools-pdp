...
import org.onap.policy.drools.server.restful.RestServerPool;
...
public class Server implements Comparable<Server> {
     ...
     */
    public static String startup(String propertiesFile) throws IOException {
    ...
    private static String socketAddressToName(InetSocketAddress dest) {
        ...
        for (String hostName : hostList) {
            try {
                ...
                logger.error("Server.socketAddressToName: error match socket address", e);
            }
        }
     ...
     */
    public static void adminRequest(byte[] data) {
     ...
     */
    public static void pingHosts(PrintStream out, String hosts) {
        ...
        for (String host : hosts.split(",")) {
            try {
                ...
                logger.error("Server.pingHosts: bad port value", e);
                error = true;
            } catch (UnknownHostException e) {
                out.println(host + ": Unknown host");
                logger.error("Server.pingHosts: unknown host", e);
                error = true;
            }
        }
        if (!error) {
            ...
            try {
                ...
                for (InetSocketAddress host : hosts) {
                    ...
                    try {
                        ...
                        logger.error("Server.pingHosts: Unable to create client connection", e);
                    } catch (NoSuchFieldException | IllegalAccessException e) {
                        out.println(host + ": Unable to get link to target");
                        logger.error("Server.pingHosts: Unable to get link to target", e);
                    } catch (Exception e) {
                        out.println(host + ": " + e);
                        logger.error("Server.pingHosts: error during ping host", e);
                    }
                    if (client != null) {
                        client.shutdown();
                    }
                ...
                logger.error("Server.pingHosts: Unable to generate 'ping' data", e);
            }
            return 0;
        };
