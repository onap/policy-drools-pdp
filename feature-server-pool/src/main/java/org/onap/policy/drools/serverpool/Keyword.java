...
public class Keyword {
    ...
    public static String lookupKeyword(Object obj) {
        ...
        if (lu == null) {
            ...
            for (Class<?> testClass : classToLookup.keySet()) {
                if (testClass.isAssignableFrom(thisClass)
                        && nullLookup != classToLookup.get(testClass)
                        && (matchingClass == null
                            || matchingClass.isAssignableFrom(testClass))) {
                    // we have a first match, or a more specific match
                    matchingClass = testClass;
                }
            }
            ...
            if (matchingClass == null) {
                ...
                if ((lu = buildReflectiveLookup(thisClass)) == null) {
                    lu = nullLookup;
                }
            } else {
                // use the same procedure as the existing 'matchingClass'
                lu = classToLookup.get(matchingClass);
            }
    ...
    /**
     * These is the interface that must be implemented by object's in the
     * 'classToLookup' table.
     */
    ...
    private static synchronized Lookup buildReflectiveLookup(Class<?> clazz) {
        ...
        // failure - no matching class name found
        return null;
    }
    ...
    private static class FieldLookup extends ReflectiveLookup {
        ...
        public String getKeyword(Object obj) {
            try {
                ...
                logger.error("Keyword.FieldLookup error: field={}, error={}",
                             field, e);
                return null;
            }
        }
    ...
    private static class MethodLookup extends ReflectiveLookup {
        ...
        public String getKeyword(Object obj) {
            try {
                ...
                logger.error("Keyword.MethodLookup error: method={}, error={}",
                             method, e);
                return null;
            }
        }
    ...
    private static class ConversionFunctionLookup implements Lookup {
        ...
        /**
         * Constructor - create an object that applies a conversion function.
         */
        ConversionFunctionLookup(String functionName) {
