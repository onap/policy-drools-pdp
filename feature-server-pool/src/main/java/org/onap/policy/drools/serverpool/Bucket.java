...
public class Bucket {
    ...
    private static LinkedList<Backup> backupList = new LinkedList<>();
    ...
    private HashMap<Class, Object> adjuncts = new HashMap<>();
    ...
    public static boolean isKeyOnThisServer(String key) {
        ...
        Bucket bucket = indexToBucket[bucketNumber];
        synchronized (bucket) {
            return bucket.owner == Server.getThisServer();
        }
    }
                        ...
                        if (bucket.owner != newOwner) {
                            ...
                            synchronized (bucket) {
                                ...
                                if (thisServer == oldOwner) {
                                    ...
                                    if (bucket.state == null) {
                                        bucket.state =
                                            bucket.new NewOwner(true, oldOwner);
                                    } else {
    ...
    public static void bucketMessage(
        ...
        if (server == null) {
            ...
            Entity<String> entity =
                Entity.entity(new String(message), MediaType.TEXT_PLAIN);
    ...
    public static void bucketResponse(
        final PrintStream out, Integer bucket, String keyword, byte[] message) {
        ...
        if (server != thisServer) {
            ...
            // the originating server, or this operation was triggered while in
            // a transient state
    ...
    static void sessionData(int bucketNumber, UUID dest, int ttl, byte[] data) {
        ...
        if (dest != null && !dest.equals(Server.getThisServer().getUuid())) {
            ...
            if ((ttl -= 1) > 0
                ...
                logger.error("Couldn't forward 'bucket/sessionData' to uuid {}",
                             dest);
            }
            return;
        }
        ...
        synchronized (bucket) {
            if (bucket.state == null) {
                ...
                logger.info("Bucket {} session data received unexpectedly",
                            bucketNumber);
                bucket.state = bucket.new NewOwner(false, bucket.owner);
            }
                ...
                synchronized (bucket) {
                    ...
                    if (bucket.owner == server) {
                        ...
                        if (bucket.owner == null) {
                            ...
                            bucket.owner = bucket.secondaryBackup;
                            bucket.secondaryBackup = null;
                        }
                    }
                    if (bucket.primaryBackup == server) {
                        // the failed server was a primary backup to this bucket --
                        // mark the entry as 'null'
                        bucket.primaryBackup = null;
                        changes = true;
                    }
                    if (bucket.secondaryBackup == server) {
                    ...
                    if (bucket.owner == thisServer && bucket.state == null) {
                        ...
                        bucket.state = bucket.new NewOwner(false, null);
                        changes = true;
                    }
    ...
    private static class TestServer {
        ...
        TreeSet<TestBucket> primaryBackupBuckets = new TreeSet<>();

        // the set of all 'TestBucket' instances,
        // where this 'TestServer' is listed as 'secondaryBackup'
        TreeSet<TestBucket> secondaryBackupBuckets = new TreeSet<>();
    ...
    private static class Rebalance {
        ...
        void copyData() {
                    ...
                    */
                    ...
                    public Integer call() {
                        ...
                        for (int i = 0 ; i < indexToBucket.length ; i += 1) {
                            ...
                            tmpBucket.owner = bucket.owner;
                            tmpBucket.primaryBackup = bucket.primaryBackup;
                            tmpBucket.secondaryBackup = bucket.secondaryBackup;
                        ...
                        for (Server server : Server.getServers()) {
                            ...
                            testServers.put(uuid,
                                new TestServer(uuid, server.getSiteSocketAddress()));
                        }
            ...
            for (Bucket bucket : bucketSnapshot) {
                ...
                if (bucket.owner != null) {
                    testBucket.setOwner(testServers.get(bucket.owner.getUuid()));
                } else {
    ...
    private class OldOwner extends Thread implements State {
        ...
        public void run() {
            ...
            try {
                ...
                LinkedList<Restore> restoreData = new LinkedList<>();
