 ...
 * PolicyEngineFeatureApi - the 'afterStart' hook is where we initialize
 * PolicyControllerFeatureApi - the 'beforeOffer' hook is used to look
...
public class FeatureServerPool
    ...
    private static long droolsTimeout;
    ...
    public boolean beforeOffer(final PolicyController controller,
        ...
        try {
            jsonObject = coder.decode(event, JsonObject.class);
        ...
        for (String[] path : table) {
            ...
            // The last entry in 'path' is the name of the actual String field,
            // while the other entries correspond to named 'JSONObject' instances
            // (name/value pairs). The following loop traverses down the tree of
            // 'JSONObject' instances -- the 'a.b.c.d' part of 'a.b.c.d.e'.
            int end = path.length - 1;
            for (int i = 0 ; i < end ; i += 1) {
                if ((element = jo.get(path[i])) == null || !element.isJsonObject()) {
                    // couldn't find field -- this 'path' does not exist in 'event'
                    jo = null;
                    break;
                }
            ...
            if (jo != null) {
                ...
                element = jo.get(fieldName);
                if (element != null && element.isJsonPrimitive()
                        && (keyword = element.getAsJsonPrimitive().getAsString()) != null) {
                    ...
                    keyword =
                        Keyword.convertKeyword(keyword, conversionFunctionName);
                    if (keyword != null) {
        ...
        if (Bucket.isKeyOnThisServer(keyword)) {
            ...
            if (webTarget != null) {
                ...
                Entity<String> entity =
                    Entity.entity(new String(data), MediaType.APPLICATION_OCTET_STREAM_TYPE);
                webTarget
                .queryParam("keyword", keyword)
    ...
    private static void buildKeywordTable() {
        ...
        while (pn.hasMoreElements()) {
            ...
            if (obj instanceof String) {
                // the property name should always be a String,
                // but the iterator provides it as an 'Object'
    ...
    private static class TopicMessage implements Bucket.Message {
        ...
        /**
         * Constructor - store the arguments as instance variables.
         *
         * @param keyword the keyword associated with this message
         * @param controller the controller receiving this message
         * @param protocol UEB or DMAAP
         * @param topic UEB or DMAAP topic
         * @param event JSON message as a String
         */
        TopicMessage(String keyword, PolicyController controller,
    ...
    static class DroolsSessionRestore implements Bucket.Restore, Serializable {
        ...
        LinkedList<SingleSession> sessions = new LinkedList<>();
        ...
        boolean backup(int bucketNumber) {
            ...
            for (PolicyContainer pc : PolicyContainer.getPolicyContainers()) {
                for (PolicySession session : pc.getPolicySessions()) {
                    ...
                    kieSession.insert(new DroolsRunnable() {
                        @Override
                        public void run() {
                            ArrayList<Object> droolsObjects = new ArrayList<>();
            ...
            long endTime = System.currentTimeMillis() + droolsTimeout;
        ...
        public void restore(int bucketNumber) {
            ...
            try {
                for (CountDownLatch sessionLatch : sessionLatches) {
                    if (!sessionLatch.await(droolsTimeout, TimeUnit.MILLISECONDS)) {
