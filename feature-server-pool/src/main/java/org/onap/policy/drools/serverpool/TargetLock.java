...
public class TargetLock implements Lock, Serializable {
    ...
    public TargetLock(final String key, final String ownerKey,
         ...
         */
        Bucket.forwardAndProcess(key, new Message() {
            /**
             * {@inheritDoc}
             */
            @Override
     ...
     */
    public static Response incomingLock(String key, String ownerKey, UUID uuid, boolean waitForLock, int ttl) {
     ...
     * @return the Response that should be passed back to the HTTP request
     */
    public static Response incomingFree(String key, String ownerKey, UUID uuid, int ttl) {
        if (!Bucket.isKeyOnThisServer(key)) {
            /**
             * this is the wrong server -- forward to the correct one
             * (we can use this thread)
     ...
     * @return the Response that should be passed back to the HTTP request
     */
    public static Response incomingLocked(String key, String ownerKey, UUID uuid, int ttl) {
        if (!Bucket.isKeyOnThisServer(ownerKey)) {
            /**
             * this is the wrong server -- forward to the correct one
             * (we can use this thread)
    ...
    private static class LockEntry implements Serializable {
        ...
        private void notifyNewOwner(final GlobalLocks globalLocks) {
            Bucket.forwardAndProcess(currentOwnerKey, new Message() {
                /**
                 * {@inheritDoc}
                 */
                @Override
     ...
     */
    static class LockBackup implements Backup {
        /**
         * {@inheritDoc}
         */
        @Override
     ...
     */
    static class LockRestore implements Restore, Serializable {
    ...
    public static class DumpLocks {
         ...
         *     line length)
         */
        public static void dumpLocks(PrintStream out, boolean detail)
         ...
         */
        public static byte[] dumpLocksData(UUID serverUuid, int ttl) throws IOException {
    ...
    public static class Audit {
         ...
         * @param serverUuid the UUID of the intended destination server
         * @param ttl similar to IP time-to-live -- it controls the number of hops
         * @param encodedData base64-encoded data, containing a serialized 'AuditData'
         *     instance
         * @return a serialized and base64-encoded 'AuditData' response
         */
        public static byte[] incomingAudit(UUID serverUuid, int ttl, byte[] encodedData) {
            if (!Server.getThisServer().getUuid().equals(serverUuid)) {
                if ((ttl -= 1) > 0) {
                    Server server = Server.getServer(serverUuid);
                    if (server != null) {
                        ...
                        if (webTarget != null) {
                            ...
                            Entity<String> entity =
                                Entity.entity(new String(encodedData, StandardCharsets.UTF_8),
