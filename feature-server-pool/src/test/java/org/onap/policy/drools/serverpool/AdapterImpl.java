...
public class AdapterImpl extends Adapter {
    ...
    public void init(int index) throws Exception {
        ...
        prop.setProperty("discovery.servers", "127.0.63.250");
        prop.setProperty("discovery.topic", "DISCOVERY-TOPIC");
        prop.setProperty("server.ipAddress", "127.0.63." + index);
        prop.setProperty("server.port", "20000");
        ...
        // 'FeatureServerPool.droolsTimeout'
        Whitebox.setInternalState(FeatureServerPool.class, "droolsTimeout",
            ServerPoolProperties.DEFAULT_BUCKET_DROOLS_TIMEOUT);

        // use reflection to set private static field
        // 'FeatureServerPool.timeToLive'
        Whitebox.setInternalState(FeatureServerPool.class, "timeToLive",
        ...
        try {
            ...
            for (int i = 0 ; i < Bucket.BUCKETCOUNT ; i += 1) {
                ...
                while (bucket.getOwner() == null) {
                    Thread.sleep(Math.min(endTime - System.currentTimeMillis(),
                                          100L));
                }
            }
    ...
    public String createController() {
        ...
        try {
            ...
            properties = PropertyUtil.getProperties("config/TestController-controller.properties");
        } catch (Exception e) {
            e.printStackTrace();
