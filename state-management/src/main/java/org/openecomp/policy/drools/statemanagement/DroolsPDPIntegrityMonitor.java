...
public class DroolsPDPIntegrityMonitor extends IntegrityMonitor
{
  ...
  static public DroolsPDPIntegrityMonitor init(String configDir) throws Exception
  {
	...
	logger.info("init: PDP_INSTANCE_ID=" + consolidatedProperties.getProperty(StateManagementProperties.NODE_NAME));
	...
	try {
		...
		logger.error("init: Caught Exception attempting to start server on hostPort="
