...
import org.onap.policy.drools.healthcheck.HealthCheckMonitor.Factory;
...
public class HealthCheckTest {
    ...
    /**
     * Name of the "factory" field within the {@link HealthCheckMonitor} class.
     */
    ...
    public static void setUpBeforeClass() {
        savedFactory = Whitebox.getInternalState(HealthCheckMonitor.class, FACTORY_FIELD);
    }

    @AfterClass
    public static void tearDownAfterClass() {
        Whitebox.setInternalState(HealthCheckMonitor.class, FACTORY_FIELD, savedFactory);
    }
    ...
    public void setUp() throws Exception {
        ...
        when(servletFactory.build(any(Properties.class))).thenReturn(servers);
        when(clientFactory.build(any(Properties.class))).thenReturn(clients);
        when(factory.getEngineManager()).thenReturn(engineMgr);
        when(factory.getServerFactory()).thenReturn(servletFactory);
        when(factory.getClientFactory()).thenReturn(clientFactory);
        when(engineMgr.isAlive()).thenReturn(true);

        Whitebox.setInternalState(HealthCheckMonitor.class, FACTORY_FIELD, factory);
    ...
    public void testHealthCheckMonitor_Start() {
        ...
        when(factory.getServerFactory()).thenThrow(new RuntimeException(EXPECTED));
