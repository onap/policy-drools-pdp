...
public class PoolingManagerImpl implements PoolingManager, TopicListener {
    ...
    public ScheduledFuture<Boolean> schedule(long delayMs, StateTimerTask task) {
        return (ScheduledFuture<Boolean>) scheduler.schedule(new TimerAction(task), delayMs, TimeUnit.MILLISECONDS);
    }

    @Override
    @SuppressWarnings("unchecked")
    public ScheduledFuture<Boolean> scheduleWithFixedDelay(long initialDelayMs, long delayMs, StateTimerTask task) {
        return (ScheduledFuture<Boolean>) scheduler.scheduleWithFixedDelay(new TimerAction(task), initialDelayMs,
    ...
    private boolean handleEvent(Forward event) {
        int bucket = (event.getRequestId().hashCode() & Integer.MAX_VALUE) % assignments.size();
