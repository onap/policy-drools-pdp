...
/**
 * Resource locks. Each lock has an "owner", which is intended to be unique
 * across a single instance of a running PolicyEngine.
 * <p>
 ...
 * <p>
 * Implementers may choose to implement a level of locking appropriate to the
 * application. For instance, they may choose to implement an engine-wide
 * locking scheme, or they may choose to implement a global locking scheme
 * (e.g., through a shared DB).
 */
public interface PolicyResourceLockFeatureAPI extends OrderedService {
    ...
    public enum BeforeLockResult {
        ...
        /**
         * The lock was handled by the implementer, and the result will be
    ...
    /**
     * Callback that an implementer invokes when a lock is acquired
     * asynchronously. The implementer invokes <i>run()</i> once the lock has
     * been acquired. In addition, immediately after invoking <i>run()</i>, the
     * implementer should invoke <i>isCancelled()</i>, to see if the request had
     * been previously cancelled. If so, then the implementer should immediately
     * release the lock. Furthermore, at any point, if the implementer sees that
     * the request has been cancelled, then the implementer may choose to
     * immediately stop trying to acquire the lock.
     */
    ...
    /**
     * This method is called before a lock is acquired on a resource. If a
     * callback is provided, and the implementer is unable to acquire the lock
     * immediately, then the implementer will invoke the callback's <i>run()</i>
     * method once the lock is acquired. Note: the callback will only be invoked
     * if this method returns ASYNC.
     * 
     * @param resourceId
     * @param owner
     * @param callback function to invoke, if the requester wishes to wait for
     *        the lock to come available, {@code null} to provide immediate
     *        replies
     ...
     *        otherwise
     * @return {@code true} if the implementer handled the request,
    ...
    /**
     * This method is called before a check is made to determine if a particular
     * owner holds the lock on a resource.
     * 
     * @param resourceId
     * @param owner
     * @return
