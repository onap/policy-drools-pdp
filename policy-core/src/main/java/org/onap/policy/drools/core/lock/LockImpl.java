...
public class LockImpl implements Lock {
     ...
     */
    public LockImpl(Lock.State state, String resourceId, Object ownerInfo, String ownerKey,
                    int holdSec, LockCallback callback, boolean succeed) {
    ...
    @Override
    public boolean free() {
        if (succeed && state == Lock.State.ACTIVE) {
            state = Lock.State.UNAVAILABLE;
            return true;
        }
    ...
    public void extend(int holdSec, LockCallback callback) {
        ...
        if (succeed && state != Lock.State.UNAVAILABLE) {
            notifyAvailable();
