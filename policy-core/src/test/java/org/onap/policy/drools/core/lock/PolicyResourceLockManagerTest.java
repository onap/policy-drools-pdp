...
public class PolicyResourceLockManagerTest {
    ...
    /**
     * Tests getProvider2() when there are no enabled providers.
     */
    @Test
    public void getProvider2NoProviders() {
        when(provider1.enabled()).thenReturn(false);
        when(provider2.enabled()).thenReturn(false);

        assertTrue(mgr.getProvider2() instanceof AlwaysFailProvider);
    }
