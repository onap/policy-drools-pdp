...
public class PolicyResourceLockManagerTest {
    ...
    @Test
    public void testLock_Async() {
        mgr.lock(RESOURCE_A, OWNER1, callback1);
        fut = mgr.lock(RESOURCE_A, OWNER2, callback2);

        assertFalse(fut.isDone());

        // should only be called during the first lock
        verify(impl1).afterLock(anyString(), anyString(), anyBoolean());
        verify(impl2).afterLock(anyString(), anyString(), anyBoolean());
    }
    ...
    @Test
    public void testImplFutureSetLocked() {

        mgr.lock(RESOURCE_A, OWNER1, null);

        // queue up asynchronous lock
        mgr.lock(RESOURCE_A, OWNER2, callback2);

        // these should not have been called yet
        verify(impl1).afterLock(RESOURCE_A, OWNER1, true);
        verify(impl2).afterLock(RESOURCE_A, OWNER1, true);
        verify(impl1, never()).afterLock(RESOURCE_A, OWNER2, true);
        verify(impl2, never()).afterLock(RESOURCE_A, OWNER2, true);

        // when owner1 unlocks, owner2 should get the lock
        mgr.unlock(RESOURCE_A, OWNER1);

        // these should be unchanged from the first lock request
        verify(impl1).afterLock(RESOURCE_A, OWNER1, true);
        verify(impl2).afterLock(RESOURCE_A, OWNER1, true);

        // afterLock() should have been called by now
        verify(impl1).afterLock(RESOURCE_A, OWNER2, true);
        verify(impl2).afterLock(RESOURCE_A, OWNER2, true);
    }
