...
public class PolicyResourceLockManagerTest {
    ...
    public void setUp() {
        ...
        when(provider2.lock(any(), any(), any(), anyInt(), anyBoolean())).thenReturn(lock1).thenReturn(lock2)
                        .thenReturn(lock3);
    ...
    public void testLock() {
        assertSame(lock1, mgr.lock(RESOURCE, OWNER, callback, HOLD_SEC, true));

        // repeat - should use the cached provider
        assertSame(lock2, mgr.lock(RESOURCE, OWNER, callback, HOLD_SEC, false));
        assertSame(lock3, mgr.lock(RESOURCE, OWNER, callback, HOLD_SEC, true));
    }

    @Test
    public void testLock_Ex() {
        // provider throws an exception
        when(provider2.lock(any(), any(), any(), anyInt(), anyBoolean()))
    ...
    protected void verifyLock() {
        LockImpl lock = (LockImpl) mgr.lock(RESOURCE, OWNER, callback, HOLD_SEC, true);
